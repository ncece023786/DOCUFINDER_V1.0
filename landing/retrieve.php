...
if(isset($_POST['button_recovery'])) {
    global $link;
    $password = $link->real_escape_string($_POST['password']);
    $confirm_password = $link->real_escape_string($_POST['confirm_password']);
    $security_code = base64_decode($_SESSION['security_code']);
    if($password != $confirm_password || $confirm_password != $password) {
        $message = base64_encode(urlencode('Password and Confirm Password mismatched.'));
        header('location: retrueve.php?success=false&security_code='.base64_encode($security_code).'&message='.$message);
    } else {
        $checking = mysqli_query($link,"SELECT * FROM accounts_tbl WHERE security_code = '$security_code' ");
        $row = mysqli_fetch_assoc($checking);
        if(mysqli_num_rows($checking) > 0) {
            $new_password = password_hash($password,PASSWORD_DEFAULT);
            $random = rand(111111,999999);
            mysqli_query($link,"UPDATE accounts_tbl SET password = '$new_password', security_code = $random WHERE security_code = '$security_code' ");
            unset($_SESION['security_code']);
            $message = base64_encode(urlencode('You have successfully retrieved your account.'));
            ...
            <div class="row g-0">
              <div class="col-lg-6">
                <div class="card-body p-md-5 mx-md-4">
                  <?php if(@$_GET['success'] == 'false' && $_GET['message']) { ?>
                  ...
                  <form method="POST">
                    <h5 style="text-align: center;">Password Recovery</h5>
                    ...
                    <div class="form-outline mb-4">
                      ...
                      <input type="password" id="form2Example11" name="confirm_password" class="form-control" />
                    </div>

                    <div class="text-center pt-1 mb-5 pb-1">
                      <button class="btn btn-danger btn-block fa-lg gradient-custom-2 mb-3" type="submit"
                        name="button_recovery" style="height: 50px; border: none;">Change My Password</button>
                    </div>
              ...
              <div class="col-lg-6 d-flex align-items-center gradient-custom-2">
                <div class="text-white px-3 py-4 p-md-5 mx-md-4">
                  ...
                  <p class="small mb-0">Ang layunin nito ay mag silbing pahina na makukuhanan ng balita't Impormasyon,
                    at mga pangyayaring nagaganap sa ating Barangay.</p>
                </div>
              </div>
            </div>
          </div>
