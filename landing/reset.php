...
if(isset($_POST['button_security'])) {
  global $link;
  $security_code = $link->real_escape_string($_POST['security_code']);
  ...
  if(mysqli_num_rows($checking) > 0) {
    ...
    header('location: retrieve.php?success=true');
  } else {
    $message = base64_encode(urlencode('Invalid security code.'));
    header('location: reset.php?success=false&security_code='.$security_code.'&message='.$message);
  }
}
?>
<!DOCTYPE html>
                ...
                <div class="card-body p-md-5 mx-md-4">
                  ...
                  <form method="POST">
                    ...
                    <div class="form-outline mb-4">
                      <label class="form-label" for="form2Example11">Security Code</label>
                      <input type="text" id="form2Example11" name="security_code"  value="<?=isset($_GET['security_code']) ? base64_decode($_GET['security_code']) : ''?>" class="form-control" />
                    </div>

                    <div class="text-center pt-1 mb-5 pb-1">
                      <button class="btn btn-danger btn-block fa-lg gradient-custom-2 mb-3" type="submit"
                        name="button_security" style="height: 50px; border: none;">Next</button>
                    </div>
